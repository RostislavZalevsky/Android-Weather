package rostislav.weather.data;

import org.json.JSONException;
import org.json.JSONObject;

public class Data implements JSONData {
    private Units units;
    private Item item;
    private String location;

    public Units getUnits() {
        return units;
    }
    public Item getItem() {
        return item;
    }
    public String getLocation() {
        return location;
    }

    @Override
    public void JSONData(JSONObject data) {

        units = new Units();
        units.JSONData(data.optJSONObject("units"));

        item = new Item();
        item.JSONData(data.optJSONObject("item"));

        JSONObject locationData = data.optJSONObject("location");

        String region = locationData.optString("region");
        String country = locationData.optString("country");

        location = String.format("%s, %s", locationData.optString("city"), (region.length() != 0 ? region : country));
    }

    @Override
    public JSONObject toJSONData() {

        JSONObject data = new JSONObject();

        try {
            data.put("units", units.toJSONData());
            data.put("item", item.toJSONData());
            data.put("requestLocation", location);
        } catch (JSONException e) {
            e.printStackTrace();
        }

        return data;
    }

}
